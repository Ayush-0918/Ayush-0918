<!-- ===================== Animated Typing Intro ===================== -->
<p align="center">
  <img 
    src="https://readme-typing-svg.herokuapp.com?size=26&duration=2500&color=58A6FF&center=true&vCenter=true&width=900&lines=Hi+👋+I'm+Ayush+Pandey;Cloud+%26+Data+Engineer+in+Training;AWS+•+Python+•+SQL;Building+Scalable+Cloud+and+Data+Systems" 
  />
</p>

---

## 👨‍💻 About Me

🎓 **B.Tech Computer Science Engineering Student**  
🏫 **CT Institute of Engineering, Management & Technology**  
💡 **Cloud & Data Engineer in Training | AWS • Python • SQL**

🔹 Interested in building **scalable cloud architectures and data pipelines**  
🔹 Focused on **cloud computing, data engineering fundamentals, and system design**  
🔹 Actively working on **hands-on projects and industry-aligned simulations**

📌 Currently strengthening skills in **AWS, Python, SQL, and data-driven systems**  
🚀 Goal: Contribute to **production-grade cloud & data platforms**

---

## 🧠 Core Skills (Cloud + Data Focused)

- **Cloud Computing:** AWS (Architecture fundamentals, storage, compute)  
- **Data Engineering:** Data pipelines, analytics workflows  
- **Programming:** Python, SQL  
- **Foundations:** Linux, Git & GitHub  
- **Problem Solving:** Structured & data-oriented thinking  

---

## 🎓 Certifications & Professional Learning

### ☁️ Cloud & Data
- Google Cloud – Introduction to Generative AI  
- Google Cloud – Large Language Models  
- Prompt Design in Vertex AI  

### 🏢 Industry Simulations (Forage)
- Tata GenAI Powered Data Analytics  
- Deloitte Australia – Technology Job Simulation  
- Citi – Technology Software Development  

---

## 🏆 Hackathons & Experience

### 🚀 DevCreate BuildFest 1.0  
🏢 **GDG Jalandhar × CT Group of Institutions**

- 🏅 Top 16 among 150+ teams  
- ⏱️ 30 hours of rapid development  
- 🤝 Team collaboration & problem solving  

---

## 🚀 Featured Projects

### 🌱 Plant Doctors  
AI-based plant health & disease detection system.  
**Tech:** Python • Machine Learning • Computer Vision  

---

### 📚 Smart Study Material Sharing Platform  
Cloud-backed academic resource management system.  
**Tech:** HTML • CSS • Backend Logic • Cloud Storage  

---

## 🧩 Coding Profiles

- 💡 **LeetCode:** https://leetcode.com/ayushpandey1112  
- 🧩 **HackerRank:** https://www.hackerrank.com/@ayushpandey10851  

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Ayush-0918&show_icons=true&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Ayush-0918&theme=tokyonight&hide_border=true" />
</p>

---

## 🌐 Connect With Me

- 💼 **LinkedIn:** https://www.linkedin.com/in/ayush-pandey-8815142b1  
- 📧 **Email:** ayushpandey10851@gmail.com  

---
         📬 *Open to internships, collaborations, and learning opportunities.*
